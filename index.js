...
function indekseGoreFenomen(fenomenlerListe, objeIndeksi) {
  return objeIndeksi + ". indekste bulunan fenomen: " + fenomenler[objeIndeksi].profile;
}
...
function profilListesi(fenomenDizisi) {
  let yeniDizi = [];
  for (let i=0; i < fenomenDizisi.length; i++){
    yeniDizi.push(fenomenDizisi[i].profile);
  }
  return yeniDizi;
}
...
function fenomenSil(eskiDizi, dizinSayisi) {
  let yeniDizi = [];
  for (let i=0; i<eskiDizi.length; i++){
    if (i===dizinSayisi){
      continue;
    } else{
      yeniDizi.push(eskiDizi[i]);
    }
  }  
  return yeniDizi;
}
...
function fenomenEkle(originalArray, number, profile, followers, posts, platform) {
  let newArray = [];
  ...
  newObject.posts=posts;
  newObject.platform=platform;
...
function enFenomenler(fenomenlerListe) {
  let yeniListe = [];
  for(let i=0; i <fenomenlerListe.length; i++){
    if(fenomenlerListe[i].followers > 100000000){
      yeniListe.push(fenomenlerListe[i].profile);
    }
  }
  return yeniListe;
}
...
function fenomenGonderimSayisi(fenomenlerListe, profilAdi){
  let postSayisi;
  for(let i=0; i<fenomenlerListe.length; i++){
    if(fenomenlerListe[i].profile === profilAdi){
      postSayisi = fenomenlerListe[i].posts;
    }
  }
  return postSayisi;
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenlerListe, platformAdi){
  const enAktifFenomen = {
    posts : 0
  }
  for (let i=0; i<fenomenlerListe.length; i++){
    if (platformAdi === fenomenlerListe[i].platform){
      if(fenomenlerListe[i].posts > enAktifFenomen.posts){
        enAktifFenomen.posts = fenomenlerListe[i].posts;
        enAktifFenomen.profile = fenomenlerListe[i].profile;
        //enAktifFenomen.platform = fenomenlerListe[i].platform;
      }
    }
  }
  return enAktifFenomen.profile;
}
